...
Live **https://dtll.org** and **https://www.dtll.org** are served by Worker **`dtll-cms`** (custom domains). Do **not** attach those domains to `crew-hq` or any other Worker. Admin CRUD writes to Supabase `site_jobs`; Careers loads them via `GET /api/jobs`.
...
Careers fetches `/api/jobs` in `js/main.js` with `cache: "no-store"` and paints that list. A successful CMS response always replaces the list (including an empty array when every role is archived). Hardcoded fallback jobs are used only if the API request fails.
...
Live custom domains already point at Worker `dtll-cms`. Pages `dtll-careers` stays a pages.dev preview and will not overwrite dtll.org.
